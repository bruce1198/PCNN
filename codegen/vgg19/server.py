		...
		if key == 'get':
			...
			if block_id == 0:
				...
				if idx == 0:
					...
					y = torch.cat((x[:, :, 0:4, :], x[:, :, 8:12, :]), dim=2)
				elif idx == 2:
					y = torch.cat((x[:, :, 4:8, :], x[:, :, 12:16, :]), dim=2)
				elif idx == 3:
					y = torch.cat((x[:, :, 8:12, :], x[:, :, 16:20, :]), dim=2)
				elif idx == 4:
					y = torch.cat((x[:, :, 12:16, :], x[:, :, 20:24, :]), dim=2)
				elif idx == 5:
					y = torch.cat((x[:, :, 16:20, :], x[:, :, 24:28, :]), dim=2)
				elif idx == 6:
					y = x[:, :, 20:24, :]
			elif block_id == 2:
				if idx == 0:
